...
const int tonePin = 7;
const int toneFrequency = 2700;
const int toneDuration = 25;

const int signalLedPin = 8;

const int vlotagePin = A4;
const float voltageMultiplier = 1.0;

const int keysRowPins[] = {4, 5, 6, -1};
const int keysColumnPins[] = {4, 5, 6};
const int keysRowSize = sizeof(keysRowPins) / sizeof(int);
const int keysColumnSize = sizeof(keysColumnPins) / sizeof(int);

const int switchsPins[] = {3, 2, A5};
const int switchsSize = sizeof(switchsPins) / sizeof(int);
int lastSwithsState[switchsSize];
...
void setup() {
  ...
  for (int i = 0; i < switchsSize; i++) {
    ...
    auxsState[i] = lastSwithsState[i] = digitalRead(switchsPins[i]);
  }
  ...
  joystickLeftYOffset = eepromRead(0);
  ...
  FlexiTimer2::set(1, updateService);
  FlexiTimer2::start();
}
...
void loop() {
  ...
  int joystickLeftY = constrain(analogRead(A0) + joystickLeftYOffset * joystickOffsetStep, 0, 1023);
  ...
  int valueRoll, valuePitch, valueThrottle, valueYaw;

  if (throttleHand == 0) {
    valueRoll = map(joystickRightY, 0, 1023, 2011, 988);
    ...
    valueRoll = map(joystickLeftY, 0, 1023, 988, 2011);
  ...
  for (int i = 0; i < auxsSize; i++) {
    bitWrite(rf24OutData[8], i, auxsState[i]);
  }
...
float getVoltage() {
  return analogRead(vlotagePin) * 5.0 / 1023 * (10 + 6.2) / 6.2 * voltageMultiplier;
}
...
void handleSwitch() {
  ...
  for (int i = 0; i < switchsSize; i++) {
    ...
    if (swithsState[i] != lastSwithsState[i]) {
      ...
      auxsState[i] = lastSwithsState[i] = swithsState[i];
